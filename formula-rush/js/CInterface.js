function CInterface(){
    ...
    this._init = function(){          
		...
		var oSprite_but_next_map = s_oSpriteLibrary.getSprite('but_next_map');
        window["oSprite_but_next_map"] = oSprite_but_next_map;
        window["but_next_mapScale"] = 1;
        _pStartPosGUI = { x: CANVAS_WIDTH - (oSprite.width / 2) - 10, y: (oSprite.height / 2) + 10 };

        _oButNextMap = new CGfxButton(
            // CANVAS_WIDTH / 2 + oSprite_but_next_map.width * window["but_next_mapScale"] / 2 + 200,
            CANVAS_WIDTH - oSprite_but_next_map.width * window["but_next_mapScale"] / 2 - 200,
            oSprite_but_next_map.height * window["but_next_mapScale"] / 2 + 10,

            oSprite_but_next_map, s_oStage, window["but_next_mapScale"]);
        _oButNextMap.addEventListener(ON_MOUSE_UP, this._onNextMap, this);
	...
	this._onNextMap = (ON_MOUSE_UP, function () {
    ...
    this.refreshButtonPos = function(){
		...
		_oButNextMap.setPosition(
            // CANVAS_WIDTH / 2 + window["oSprite_but_next_map"].width * window["but_next_mapScale"] / 2 + 200 + s_iOffsetX, 
            CANVAS_WIDTH - window["oSprite_but_next_map"].width * window["but_next_mapScale"] / 2 - 200 - s_iOffsetX,
            window["oSprite_but_next_map"].height * window["but_next_mapScale"] / 2 + 10 + s_iOffsetY
        );
