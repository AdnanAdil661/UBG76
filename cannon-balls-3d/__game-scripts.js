...
InputController.prototype.handleTouch = function (x, y) {
    this.app.fire("input:down", { x: x, y: y / this.app.graphicsDevice.canvas.height * window.innerHeight * ScaleManager.qualityFactor });
};
        ...
        Apicontroller.handleLevelEndEvent("success", ScoreManager.instance.getScores(), () => {
            if (isForcedMode()) {
                ...
                Apicontroller.trackLevelEnd({
                    ...
                    "movesLeft": this.app.root.findByName('BallsBox').script.ballsManager.ballsLeft,
                    "movesAvailable": this.app.root.findByName('BallsBox').script.ballsManager.totalBalls,
    ...
    this.entity.show = function() {
        ...
        if(scriptContext.orientation === LevelProgressContainer.PORTRAIT) {
            ...
            this.tween(this.getLocalPosition())
                .to(new pc.Vec3(0, -140, 0), 0.4, pc.BackOut)
                .delay(0.2)
                .start();
        } else {
...
LevelProgressContainer.prototype.update = function(dt) {
    ...
    pos.x = this.bestScoreText.getLocalPosition().x - this.bestScoreText.element.width / 2 + 5;
...
BallsManager.prototype.handleViewportResized = function () {
    this.entity.delayedCall(0, () => {
        ...
        this.entity.setPosition(position.x, GameplayController.gameStarted ? position.y : position.y - 1, GameplayController.gameStarted ? position.z : position.z + 6);
        this.enableBodies(this.entity);
    });
};
...
//     if(window.famobi_analytics) {        
//         window.famobi_analytics.trackEvent("EVENT_LEVELSTART", {levelName: '' + GameplayController.currentLevel}).then(() => {
...
Apicontroller.initTracking = function () {
    ...
    window.famobi_tracking.init('cannon-balls', null, 100, true, true);
};
