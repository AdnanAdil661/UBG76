...
class k1FAB extends Phaser.GameObjects.Sprite {

	constructor(scene, x, y, texture, frame) {
		super(scene, x ?? 0, y ?? 0, texture || "k1RUN_sprite", frame ?? 0);
		...
		if(localStorage.getItem("_UNENDINGFAITH_UPGRADE_STATS") != null){
			...
			if(_UNENDINGFAITH_STATS != undefined){
				this.damage = Phaser.Math.Between(14, 20) + _UNENDINGFAITH_STATS; // damage 14 - 20
			}
		}
	...
	create(){
		...
		this.anims.play('k1_run');
		this.collision = false;	

		this.on("animationstart", (anim) => {
			if(anim.key == "k1_die"){ this.scene.knightDeath.play(); }
		}, this);
		...
		this.sndArray = [];
		this.sndMarker = false;
	}

	k1Collision(knight, enemy){
		this.e = enemy;
		...
		this.scene.spearSwing.play();
/*		if(this.sndMarker == false){
	...
	update(){
		if(this.health < 0) { 
			...
			this.anims.play('k1_die', false); 
			this.scene.events.off(Phaser.Scenes.Events.UPDATE, this.update, this);
		}
		if(this.e.active != true && this.body.enable == true){ this.body.setVelocityX(this.velocity); this.anims.play('k1_run', true); }
		this.collision = false;
	}
