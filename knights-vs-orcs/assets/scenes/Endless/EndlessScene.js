...
class EndlessScene extends Phaser.Scene {
	...
	editorCreate() {
		...
		// orc_castle0
		const orc_castle0 = new EnemyCastlePrefab(this, 1896, 491);
		...
		// hightlight
		const hightlight = this.add.rectangle(954, 522, 130, 108);
		...
		// hp_orc_castle
		const hp_orc_castle = this.add.sprite(1746, 224, "hp", 10);

		// arrow_t1
		const arrow_t1 = this.add.image(475, 379, "arrow_t1");
		arrow_t1.visible = false;

		// arrow_t1_1
		const arrow_t1_1 = this.add.image(466, 781, "arrow_t1");
	...
	create() {
		...
		if(_GLOBAL_COINS > 0){
			this.coinText.setText(_GLOBAL_COINS);
		}else{
		...
		// SET TIMERS:
		let timer, timer1, timer2, timer4, timer5;

		if(_ROUNDS < 3){
			timer = timers.timer_lessthan3;
			timer1 = timers.timer1_lessthan3;
			timer2 = timers.timer2_lessthan3;
			timer4 = timers.timer4_lessthan3;
			timer5 = timers.timer5_lessthan3;
		}else if(_ROUNDS < 8){
			timer = timers.timer_lessthan7;
			timer1 = timers.timer1_lessthan7;
			timer2 = timers.timer2_lessthan7;
			timer4 = timers.timer4_lessthan7;
			timer5 = timers.timer5_lessthan7;
		}else if(_ROUNDS >= 8){
			timer = timers.timer_lessthan10;
			timer1 = timers.timer1_lessthan10;
			timer2 = timers.timer2_lessthan10;
			timer4 = timers.timer4_lessthan10;
			timer5 = timers.timer5_lessthan10;
		}
		...
		this.orc_castle0.maxHealth = castleHP;
		this.orc_castle0.health = castleHP;
	...
	spawnWINwindow(){
		this.battle_music.stop();
		this.battle_win.play();
		this.scene.run("PopUpScene", { sceneName: this.scene.key, scene: this.scene, tutorialMode: false, type: "endless_win" });
		this.scene.pause();
	}
