...
class tutorial2Scene extends Phaser.Scene {
	...
	editorCreate() {
		...
		// coin_ui
		this.add.image(200, 67, "coin_ui");

		// coinText
		const coinText = this.add.text(341, 36, "", {rtl: true});
		coinText.text = "0\n";
		coinText.setStyle({"align":"right","fontFamily":"EnchantedLand","fontSize":"55px"});

		// crown_ui
		this.add.image(593, 67, "crown_ui");

		// crownText
		const crownText = this.add.text(732, 36, "", {rtl: true});
		crownText.text = "0\n";
		...
		// hightlight
		const hightlight = this.add.rectangle(937, 511, 130, 108);
		...
		// arrow_t1
		const arrow_t1 = this.add.image(458, 368, "arrow_t1");
		arrow_t1.visible = false;

		// arrow_t1_1
		const arrow_t1_1 = this.add.image(449, 770, "arrow_t1");
	...
	create() {
		...
		// SET CURRENT STAGE
		localStorage.setItem("_CURRENT_STAGE", "tutorial2Scene");
		...
		this.g2 = this.add.grid(955, 635, 1170, 216, 130, 108);
		...
		this.g2.on('pointerup', (pointer, localX, localY) => {
			...
			if(x > 4){ selectedCard = "";}
			if (x < 4 && (y== 0 || y == 1) && selectedCard.length != 0) {
		...
		//SHOW NEW CHARACTER
		this.scene.run("PopUpScene", { sceneName: this.scene.key, scene: this.scene, tutorialMode: false, type: "show_character_1"});
		this.scene.pause();
	...
	spawnWINwindow(){
		...
		this.scene.run("PopUpScene", { sceneName: this.scene.key, scene: this.scene, tutorialMode: false, type: "WIN", nextScene: "Stage1Scene3" });
		this.scene.pause();
	}
