...
class Stage4Scene1 extends Phaser.Scene {
	...
	editorCreate() {
		...
		// orc_castle0
		const orc_castle0 = new EnemyCastlePrefab(this, 1904, 519);
		...
		// hightlight
		const hightlight = this.add.rectangle(1188, 900, 130, 108);
		...
		// archer_card
		const archer_card = this.add.image(1103, 1001, "archer_card");
	...
	create() {
		// SET CURRENT STAGE
		localStorage.setItem("_CURRENT_STAGE", "Stage4Scene1");
		...
		this.scene.run("PopUpScene", { sceneName: this.scene.key, scene: this.scene, tutorialMode: false, type: "show_character_5"});
		this.scene.pause();
	}
	...
	spawnWINwindow(){
		this.battle_music.stop();
		this.battle_win.play();
		this.scene.run("PopUpScene", { sceneName: this.scene.key, scene: this.scene, tutorialMode: false, type: "WIN", nextScene: "Stage4Scene2" });
		this.scene.pause();
	}
	...
	heroesSpawn(){
		...
		this.time.addEvent({
			delay: 10000, // 
			callback: () => {
				this.scene.run("PopUpScene", { sceneName: this.scene.key, scene: this.scene, tutorialMode: false, type: "final_charge"});
				this.scene.pause();				
			},
			//args: [],
			callbackScope: this,
