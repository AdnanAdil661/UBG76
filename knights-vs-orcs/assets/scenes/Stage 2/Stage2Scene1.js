...
class Stage2Scene1 extends Phaser.Scene {
	...
	editorCreate() {
		...
		// hightlight
		const hightlight = this.add.rectangle(1150.06917475501, 863.979254606746, 130, 108);
	...
	create() {
		// SET CURRENT STAGE
		localStorage.setItem("_CURRENT_STAGE", "Stage2Scene1");
		...
		this.scene.run("PopUpScene", { sceneName: this.scene.key, scene: this.scene, tutorialMode: false, type: "show_character_2"});
		this.scene.pause();
	}
	...
	spawnWINwindow(){
		...
		this.battle_music.stop();
		this.scene.run("PopUpScene", { sceneName: this.scene.key, scene: this.scene, tutorialMode: false, type: "WIN", nextScene: "Stage2Scene2" });
		this.scene.pause();
	}
