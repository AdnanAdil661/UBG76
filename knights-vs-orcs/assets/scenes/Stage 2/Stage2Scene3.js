...
class Stage2Scene3 extends Phaser.Scene {
	...
	editorCreate() {
		...
		// elves_castle0
		const elves_castle0 = new CastlePrefab(this, -18, 488);
		this.add.existing(elves_castle0);

		// orc_castle0
		const orc_castle0 = new EnemyCastlePrefab(this, 1903, 488);
		...
		// spearman_card
		const spearman_card = this.add.image(458, 995, "spearman_card");
		...
		// hightlight
		const hightlight = this.add.rectangle(1166, 869, 130, 108);
	...
	create() {
		// SET CURRENT STAGE
		localStorage.setItem("_CURRENT_STAGE", "Stage2Scene3");
		...
		this.scene.run("PopUpScene", { sceneName: this.scene.key, scene: this.scene, tutorialMode: false, type: "show_character_3"});
		this.scene.pause();
	}
	...
	spawnWINwindow(){
		...
		this.battle_music.stop();
		this.scene.run("PopUpScene", { sceneName: this.scene.key, scene: this.scene, tutorialMode: false, type: "WIN", nextScene: "Stage3Scene1" });
		this.scene.pause();
	}
