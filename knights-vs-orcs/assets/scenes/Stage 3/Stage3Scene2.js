...
class Stage3Scene2 extends Phaser.Scene {
	...
	editorCreate() {
		...
		// hightlight
		const hightlight = this.add.rectangle(1167, 894, 130, 108);
	...
	create() {
		// SET CURRENT STAGE
		localStorage.setItem("_CURRENT_STAGE", "Stage3Scene2");
	...
	spawnLOSTwindow(){
		...
		if(getFirstVisit == "false"){
			this.scene.run("PopUpScene", { sceneName: this.scene.key, scene: this.scene, tutorialMode: false, type: "present_upgrade" });
			this.scene.pause();
		}else{
	...
	spawnWINwindow(){
		...
		if(getFirstVisit == "false"){
			...
			this.scene.run("PopUpScene", { sceneName: this.scene.key, scene: this.scene, tutorialMode: false, type: "WIN", nextScene: "Stage3Scene3" });
			this.scene.pause();			
		}
