...
class Stage3Scene1 extends Phaser.Scene {
	...
	create() {
		// SET CURRENT STAGE
		localStorage.setItem("_CURRENT_STAGE", "Stage3Scene1");
		...
		this.scene.run("PopUpScene", { sceneName: this.scene.key, scene: this.scene, tutorialMode: false, type: "show_character_4"});
		this.scene.pause();
		this.orc_castle0.maxHealth = 4000;
		this.orc_castle0.health = 4000;
	}
	...
	spawnWINwindow(){
		...
		this.scene.run("PopUpScene", { sceneName: this.scene.key, scene: this.scene, tutorialMode: false, type: "WIN", nextScene: "Stage3Scene2" });
		this.scene.pause();
	}
