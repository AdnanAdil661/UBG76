function CGame(oData,  iLevel){
    ...
    this._init = function(){
		...
		//trace("_aRunnersTeam["+_iNationalitySelected+"]:"+_aRunnersTeam[_iNationalitySelected]); 
        ...
        for( var i=0; i < NUM_RUNNERS; i++ ){
            if(i !== _iPlayerLane){
                ...
                //trace("PLAYER_NAME_AND_SPRITE["+(_aTeamPerLane[_aTeamPerLane.length-1])+"]: "+PLAYER_NAME_AND_SPRITE[_aTeamPerLane[_aTeamPerLane.length-1]])
    ...
    this.prepareRunners = function(){
        playSound("prepare_runner",1,false);
        for(var i = 0; i < NUM_RUNNERS; i++){
            _aRunners[i].playAnimation();
        }
